import React from "react";
import { AXFIconComponent, AXFIconProps } from "./AXFIconComponent";
function AXFIconCircleUploadDuo(props: AXFIconProps) {
  return (
    <AXFIconComponent {...props}>
      <svg width='16' height='16' viewBox='0 0 16 16' fill='none' xmlns='http://www.w3.org/2000/svg'>
        <path
          d='M8 15C11.866 15 15 11.866 15 8C15 4.13401 11.866 1 8 1C4.13401 1 1 4.13401 1 8C1 11.866 4.13401 15 8 15Z'
          fill='red'
          fillOpacity='0.3'
        />
        <path
          d='M8 0C3.6 0 0 3.6 0 8C0 12.4 3.6 16 8 16C12.4 16 16 12.4 16 8C16 3.6 12.4 0 8 0ZM8 15C4.1 15 1 11.9 1 8C1 4.1 4.1 1 8 1C11.9 1 15 4.1 15 8C15 11.9 11.9 15 8 15Z'
          fill='black'
        />
        <path d='M4 6H6V9H10V6H12L8 3L4 6Z' fill='black' />
        <path d='M10 10H6V11H10V10Z' fill='black' />
        <path d='M10 12H6V13H10V12Z' fill='black' />
        <path
          d='M8 0C3.6 0 0 3.6 0 8C0 12.4 3.6 16 8 16C12.4 16 16 12.4 16 8C16 3.6 12.4 0 8 0ZM8 15C4.1 15 1 11.9 1 8C1 4.1 4.1 1 8 1C11.9 1 15 4.1 15 8C15 11.9 11.9 15 8 15Z'
          fill='black'
        />
        <path d='M4 6H6V9H10V6H12L8 3L4 6Z' fill='black' />
        <path d='M10 10H6V11H10V10Z' fill='black' />
        <path d='M10 12H6V13H10V12Z' fill='black' />
      </svg>
    </AXFIconComponent>
  );
}
export { AXFIconCircleUploadDuo };
